...
;--------------------------------- PROBLEM HERE DUE TO WAY TLDS WORK
...
;ExitLoop
;Else
;If $aArray_Domains[$ai][1] = "" then
;_SQLiteInsertDomainfileDB("localfile" & $ai)
;Else
...
GUICtrlSetData($Edit1,"Domain source file no data found or no TLDS in file - " & $file & @CRLF & GUICtrlRead($Edit1))
ConsoleWrite("Domain source file no data found or no TLDS in file - " & $file & @CRLF)
Endif
...
If _SQLiteDuplicateDomainDB($NewDomainFe) = $NewDomainFe then
; Do nothing if duplicate found it will be skipped from database
GUICtrlSetData($Edit1,"Domain already processed - " & $bi & "/" & UBound($domainFound) - 1 & " - " & $NewDomainFe & @CRLF & GUICtrlRead($Edit1))
ConsoleWrite("Domain already processed - " & $bi & "/" & UBound($domainFound) - 1 & " - " & $NewDomainFe & @CRLF)
exitloop
...
_SQLite_Exec (-1, "CREATE TABLE tblSpiderNew (domain,dated,valid,checked);") ; CREATE a Table
_SQLite_Exec (-1, "CREATE TABLE tblSpiderJob (domain,title,dated,url,valid,checked);") ; CREATE a Table
_SQLite_Close()
_SQLite_Shutdown()
EndFunc
...
; OTHER BITS OF INTREST BUT NO USED OR CODED CORRECTLY
; VT Functions UDF
;Func AVExample()
;;    _Crypt_Startup()
...
;Func AVExample2()
; POST TO FACEBOOK Business Page links or groups or account whatever (You will need to get an API key from facebook and enable it for the place you want to post too.)
;Local $oHTTP,$HTMLSource,$jsObj
;$oHTTP = ObjCreate("winhttp.winhttprequest.5.1")
;$oHTTP.Open("POST","https://graph.facebook.com/YOURPAGEIDHERE/feed?message=DirectUKJobs found a job at&picture="http://innovationexcellence.com/wp-content/uploads/2016/09/googles-new-logo-5078286822539264.3-hp2x.gif"&link="http://www.google.com"&access_token=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
;$oHTTP.Send()
;$HTMLSource = $oHTTP.Responsetext
;ConsoleWrite(@LF & "Posted to Facebook = " & $srow1)
;;;;NOTE FB ID can be found with https://lookup-id.com
;EndFunc   ;==>Example2